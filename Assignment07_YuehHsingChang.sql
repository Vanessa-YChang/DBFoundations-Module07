...
-- 2020-05-27,YuehHsingChang,Created File
--**************************************************************************--
Begin Try
	Use Master;
	If Exists(Select Name From SysDatabases Where Name = 'Assignment07DB_YuehHsingChang')
	 Begin 
	  Alter Database [Assignment07DB_YuehHsingChang] set Single_user With Rollback Immediate;
	  Drop Database Assignment07DB_YuehHsingChang;
	 End
	Create Database Assignment07DB_YuehHsingChang;
End Try
Begin Catch
	Print Error_Number();
End Catch
go
Use Assignment07DB_YuehHsingChang;
...
-- CategoryName,
-- InventoryDate = DATENAME(mm, InventoryDate)+ ',' + DATENAME(yyyy,InventoryDate), 
-- InventoryCountByCategory  = [Count]
-- FROM 
-- vCategories as C 
-- INNER JOIN vProducts as P 
-- ON C.CategoryID = P.CategoryID
...
-- 	InventoryDate,  
-- 	InventoryCount,
-- 	PreviousMonthCount = LAG(InventoryCount) OVER (PARTITION BY ProductName ORDER BY Month(InventoryDate),InventoryCount)  	
-- 	FROM
-- 	vProductInventories
-- 	ORDER BY ProductName, Month(InventoryDate);
-- go 
...
AS
	...
	PreviousMonthCount = ISNull(LAG(InventoryCount) 
		OVER (PARTITION BY ProductName 
			ORDER BY Month(InventoryDate),InventoryCount),0)
	FROM
	vProductInventories
	ORDER BY ProductName, Month(InventoryDate);
go  
